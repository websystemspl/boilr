...
printf "Hello in Boilr installation wizard\n\n"
...
find . -type f \( -iname "*.php" -or -iname "*.yaml" -or -iname "*.json" \) -exec sed -i "s=Boilr\\\=${namespace}\\\=g" {} +
fi
...
read q
if [[ "${q}" == "yes" ]] || [[ "${q}" == "y" ]]; then
read -p "Enter name (without special characters and spaces, use only if name is default - boilr) : " pluginnamedirectory
